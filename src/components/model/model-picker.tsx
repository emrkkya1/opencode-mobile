...
export function ModelPicker({ value, onSelect }: ModelPickerProps) {
  ...
  const selectedModel = models.find((m) => m.id === value?.id && m.providerID === value?.providerID)
        ...
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-bg-base rounded-t-2xl max-h-[70%]">
            <View className="p-4 border-b border-border-base">
              <Text className="text-lg font-semibold text-text-strong">Select Model</Text>
            </View>
            <FlatList
              data={models}
              keyExtractor={(item) => `${item.providerID}-${item.id}`}
              renderItem={({ item }) => {
                const isSelected = item.id === value?.id && item.providerID === value?.providerID
                return (
                  <Pressable
                  ...
                  >
                    <View className="flex-1">
                      ...
                      <Text className="text-sm text-text-weak">{item.providerName}</Text>
                    </View>
